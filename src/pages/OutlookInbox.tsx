...
const OutlookInbox = () => {
  ...
  const fetchOutlookAccounts = async () => {
    ...
    if (accounts) {
      setOutlookAccounts(accounts.map(acc => ({
        id: acc.id,
        name: acc.name || acc.email,
        email: acc.email,
        unreadCount: acc.unread_count || 0,
        provider: acc.provider as 'outlook'
      })));
    }
  };
  ...
  const handleSyncAll = async () => {
    toast.loading("Syncing all Outlook accounts...", { id: "sync-all" });
    try {
      for (const account of outlookAccounts) {
        await supabase.functions.invoke("sync-outlook-messages", {
          body: { accountId: account.id, maxMessages: 100 },
        });
      }
      toast.success("All Outlook accounts synced", { id: "sync-all" });
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <ComposeDialog
        ...
        accountEmail={outlookAccounts[0]?.email || ""}
        provider="outlook"
      />
    </div>
  );
